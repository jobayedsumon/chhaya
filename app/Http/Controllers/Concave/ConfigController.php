...
class ConfigController extends Controller {
	...
	{	
        ...
        $this->data['pageNote']  =   __('core.t_generalsettingsmall');	
		$this->data['active'] = '';
		return view('concave.config.index',$this->data);	
	}
...
$val  .= 	"];\n";
			...
			fwrite($fp,$val); 
			fclose($fp); 
			return redirect('concave/config')->with('message','Setting Has Been Save Successful')->with('status','success');
		} else {
			return redirect('concave/config')->with('message', 'The following errors occurred')->with('status','success')
	...
	public function getEmail()
	{
		...
		return view('concave.config.email',$this->data);		
	...
	function postEmail( Request $request)
	{
		...
		if ($validator->passes()) 
		{
			...
			return redirect('concave/config/email')->with('message', 'Email Has Been Updated')->with('status','success');	
			
		}	else {

			return redirect('concave/config/email')->with('message', 'The following errors occurred')->with('status','success')
			->withErrors($validator)->withInput();
		}
	...
	public function getSecurity()
	{
		...
		return view('concave.config.security',$this->data);		
...
$val  .= 	"];\n";
			...
			fwrite($fp,$val); 
			fclose($fp); 
			return redirect('concave/config/security')->with('message','Setting Has Been Save Successful')->with('status','success');
		} else {
			return redirect('concave/config/security')->with('message', 'The following errors occurred')->with('status','error')
			->withErrors($validator)->withInput();
		}	
	}
	...
	public function getLog( $type = null)
	{
		...
		return view('concave.config.log',$this->data);	
	}
	...
	public function getClearlog()
	{
		...
		//return redirect('concave/config/log')->with('message','Cache has been cleared !')->with('status','success');	
	}
	...
	public function getTranslation( Request $request, $type = null)
	{
		...
		return view('concave.config.translation.'.$template,$this->data);	
	}
	
	public function getAddtranslation()
	{
		return view("concave.config.translation.create");
	} 
	...
	public function postAddtranslation( Request $request)
	{
		...
		if ($validator->passes()) {
			...
			return redirect('concave/config/translation')->with('messagetect','New Translation has been added !')->with('status','success');	;			
			
		} else {
			return redirect('concave/config/translation')->with('message','Failed to add translation !' )->with('status','error')->withErrors($validator)->withInput();
		}		
	...
	//	$filename = 'lang.php';
		...
		fwrite($fp,$form); 
		fclose($fp); 	
		return redirect('concave/config/translation?edit='.$lang.'&file='.$file)
	...
	public function getRemovetranslation( $folder )
	{
		...
		return redirect('concave/config/translation')->with('message','Translation has been removed !')->with('status','success');	
