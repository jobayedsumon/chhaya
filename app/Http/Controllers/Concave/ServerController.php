...
class ServerController extends Controller {
    ...
    function index()
    {
        ...
        return view('concave.server.index', $this->data);
    }
    function show( Request $request , $task )
    {
        switch ($task) {
            case 'repository':
                 return view('concave.server.repository', $this->data);
                break;
            case 'install':
                $this->data['id'] = $request->input('id');
                $this->data['type'] = $request->input('t');
                 return view('concave.server.authen', $this->data);
                break;    
             case 'load':
                $page = $request->input('page') ;
                $page = ( $page =='' ? '' : '&page='.$page);
                $getRows = file_get_contents('http://concave5.net/api?codename=lts'. $page ) or die ('ERROR');
                $rows = json_decode( $getRows );
                $this->data['rows'] =  $rows->rows ;
                $this->data['control'] =  $rows->control ;
                return view('concave.server.item', $this->data);
                break;           
            default:
    ...
    public function doInstall( $request) 
    {
        ...
        $message =  '';
      //  return 'http://concave5.net/api/download?id='.$id.'&uname='.$uname;
        $newInstall = file_get_contents('http://concave5.net/api/download?id='.$id.'&uname='.$uname) or die ('ERROR');
        ...
        if(count($test) >=1 ){
            ...
            if($type =='' or $type =='module') {
                $data = \ConcaveHelpers::cf_unpackage( $temp );
                $msg = '.';
    ...
    public function doUpdate( $request) {
           ...
           $path = base_path()."/resources/views/concave/server/version.json";
            ...
            {
                $message =  '';
                $getVersions = file_get_contents('http://concave5.net/upgrade?codename=lts') or die ('ERROR');
                $newUpdate = file_get_contents('http://concave5.net/upgrade/download?codename=lts&uname='.$uname);
                ...
                if(count($test) >=1 ){
                    ...
                    if(\ConcaveHelpers::upgrade( $temp))
                    {
                        $message .= ' Update successfull ! <p> <b> Concave Updated to version : </b> '. $getVersions['Version'] .'</p>' ;
                        $change_version = 
                        $fp=fopen($path,"w+");              
       ...
       foreach($rows as $row)
        {
           ...
           include(base_path().'/resources/views/concave/module/template/'.$row->module_type.'/config/route.php' );
