...
class HomeController extends Controller {
	...
	public function index( Request $request) 
	{
		...
		if(config('concave.cnf_front') =='false' && $request->segment(1) =='' ) :
			return redirect('dashboard');
		endif; 	

		//print_r( config('concave'));
		...
		if($page !='') {
			...
			$row = $sql[0];
			if(file_exists(base_path().'/resources/views/layouts/'.config('concave.cnf_theme').'/template/'.$row->filename.'.blade.php') && $row->filename !='')
			{
				$page_template = 'layouts.'.config('concave.cnf_theme').'.template.'.$row->filename;
			} else {
				$page_template = 'layouts.'.config('concave.cnf_theme').'.template.page';
			}	
			if($row->access !='')
			{
			...
			if($row->template =='frontend'){
				$page = 'layouts.'.config('concave.cnf_theme').'.index';
				...
				return view( 'layouts.default.template.homepage', compact('sliders'));
		}
	}
	...
	public function  getSkin($skin='concave')
	{
		...
		if(count($rows))
		{
			...
			if($row->method =='email')
			{
				...
				$data = array(
					'email'		=> $row->email ,
					'content'	=> $content ,
					'subject'	=> "[ " .config('concave.cnf_appname')." ] New Submited Form ",
					'title'		=> $row->name 			
				);
			
				if( config('concave.cnf_mail') =='swift' )
				{ 				
					\Mail::send('concave.form.email', $data, function ( $message ) use ( $data ) {
			    		$message->to($data['email'])->subject($data['subject']);
			    	});		

				}  else {

					$message 	 = view('concave.form.email', $data);
					$headers  	 = 'MIME-Version: 1.0' . "\r\n";
					$headers 	.= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
					$headers 	.= 'From: '. config('concave.cnf_appname'). ' < contact@vmsinc.us >' . "\r\n";
					mail(config('concave.cnf_email'), $data['subject'], $message, $headers);	
				}
	...
	public function posts( Request $request ,  $category = '') 
	{
		...
		$this->data['pages'] = 'layouts.'.config('concave.cnf_theme').'.blog.index';	
		$page = 'layouts.'.config('concave.cnf_theme').'.index';
		return view( $page , $this->data);	
	}
	...
	//	print_r($posts);exit;
		...
		$data = [
			...
			'posts'	=> $row ,
			'comments'	=>  $comments ,
			'pages' => 'layouts.'.config('concave.cnf_theme').'.blog.view',
