...
class UsersController extends Controller {
	...
	public function index( Request $request ){
		...
		if(\Auth::user()->group_id > 2){
			if(\Auth::user()->hierarchy_level == 6){
				return redirect('/dashboard')->with('status', 'error')->with('message','You are not allowed to acces this page!');
			}
		}
	...
	function create( Request $request ) 
	{
		...
		if(\Auth::user()->group_id > 2){
			...
			$disabled_levels = \DB::table('con_settings')->where('id',1)->first()->disabled_sales_team;
	...
	function edit( Request $request , $id ) 
	{
		...
		if($id == \Auth::id()){
			return redirect('dashboard')->with('message', 'You can not edit yourself. Only higher level executives can edit your profile!')->with('status','error');	
		}
	...
	function postSave( $request, $id =0)
	{
		...
		if ($validator->passes()) {
			...
			if($request->input('id') ==''){
				...
				$data['temp_pass'] = $temporaryPassword;
				$data['password'] = \Hash::make($password);
				$id = $this->model->insertRow($data , $request->input('id'));
				$user = Users::find($id);
				$fullname = $user->first_name;
				$group_id = $user ->group_id;
				$group_name = Groups::find($group_id)->name;
				
				if($group_id == 2 || $group_id == 3 || $group_id == 7){
					$articale_word = 'an';
				}else{
					$articale_word = 'a';
				}
				
				$message = 'Congratulations '.$fullname.'! You are become '.$articale_word.' '.$group_name.' of Chhaya.xyz. Use this '.$password.' code as temporary password to login. This code will expire within next 72 hours.';
			...
			if(!is_null($request->input('groups')))
			{
				...
				for($i=0; $i<count($groups); $i++)
				{
					...
					foreach($users as $row)
					{
						...
						$data['cnf_appname'] = $this->data['concaveconfig']['cnf_appname'];
