...
class UserController extends Controller {
	...
	public function getRegister() {
		...
		if(config('concave.cnf_regist') =='false') :    
			if(\Auth::check()):
	...
	public function postCreate( Request $request) {
		...
		if ($validator->passes()) {
			...
			if(config('concave.cnf_activation') == 'confirmation')
			{ 
	...
	public function reCaptcha( $request)
	{
        ...
        {
            $api_url = 'https://www.google.com/recaptcha/api/siteverify?secret=' . config('concave.cnf_recaptchaprivatekey') . '&response='.$request['g-recaptcha-response'];
	...
	public function postSignin( Request $request) {
		...
		if ($validator->passes()) {	
			...
			if (\Auth::attempt(array('username'=>$request->input('email'), 'password'=> $request->input('password') ), $remember )){
				...
				if(\Auth::check())
				{
					...
					} else if($row->active=='Banned')
					{
						...
						if(!is_null($row->temp_pass)){
							$created_time = $row->created_at;
							$currentTime = date('Y-m-d H:i:s');
							$to_time = strtotime($currentTime);
							$from_time = strtotime($created_time);
							$muniteDiff =  round(abs($to_time - $from_time) / 60,2);
							if($muniteDiff > 4320){ // 72 hours of validity for temp password finished
								if($request->ajax() == true ){
									\Auth::logout();
									return response()->json(['status' => 'error', 'message' => 'Your temporary password has been expired. Please rest your password to login!']);
								} else {
									\Auth::logout();
									return redirect('user/login')->with(['status' => 'error', 'message' => 'Your temporary password has been expired. Please rest your password to login!']);
								}

							}else{
								$firstLogin = true;
								\DB::table('tb_users')->where('id',$row->id )->update([
									'temp_pass' => 0,
						...
						if(!is_null($request->input('language')))
						{
							...
							$session['lang'] = config('concave.cnf_lang');
						...
						{
							if($firstLogin){
								return response()->json(['status' => 'success', 'url' => url('user/profile?action=pass')]);
							}else{
							...
							session($session);
							if( config('concave.cnf_front') =='false') :
								if($firstLogin){
									return redirect('user/profile?action=pass');	
								}else{
	...
	public function postRequest( Request $request)
	{
		...
		if ($validator->passes()) {	
			...
			if($user->count() >=1)
			{
				...
				$subject = "[ " .config('concave.cnf_appname')." ] REQUEST PASSWORD RESET "; 
				$data['subject'] =  $subject;	
				$data['email'] = $to;

				if(config('concave.cnf_mail') =='swift')
				{ 
					...
					$headers  = 'MIME-Version: 1.0' . "\r\n";
					$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
					$headers .= 'From: '.config('concave.cnf_appname').' <'.config('concave.cnf_email').'>' . "\r\n";
						mail($to, $subject, $message, $headers);	
				}					
