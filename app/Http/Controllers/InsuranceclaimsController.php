...
class InsuranceclaimsController extends Controller {
	...
	public $module = 'insuranceclaims';
	static $per_page	= '50';

	public function __construct()
	{		
		parent::__construct();
		$this->model = new Insuranceclaims();	
		
		$this->info = $this->model->makeInfo( $this->module);	
		$this->data = array(
			'pageTitle'	=> 	$this->info['title'],
			'pageNote'	=>  $this->info['note'],
			'pageModule'=> 'insuranceclaims',
	...
	public function index( Request $request )
	{
		...
		if(!\Auth::check()) 
			return redirect('user/login')->with('status', 'error')->with('message','You are not logged in user!');
		$this->grab( $request) ;
	...
	function create( Request $request , $id =0 ) 
	{
		...
		$subscriptions = Insurancesubscriptions::where('entry_by',\Auth::id())->get(); 
		if(count($subscriptions) == 0){
			return redirect('dashboard')->with('message', 'You have no active subscription to claim')->with('status','error');
		}else{
	...
	public static function display(  )
	{
		...
		$model  = new Insuranceclaims();
		$info = $model::makeInfo('insuranceclaims');
		$data = array(
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('insuranceclaims.public.view',$data);			
			}			
		} 
		else {
			...
			return view('insuranceclaims.public.index',$data);	
		}
