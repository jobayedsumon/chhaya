...
class InsurancepackageController extends Controller {
	...
	public $module = 'insurancepackage';
	static $per_page	= '50';

	public function __construct()
	{		
		parent::__construct();
		$this->model = new Insurancepackage();	
		
		$this->info = $this->model->makeInfo( $this->module);	
		$this->data = array(
			'pageTitle'	=> 	$this->info['title'],
			'pageNote'	=>  $this->info['note'],
			'pageModule'=> 'insurancepackage',
		...
		switch ($task)
		{
			default:
				...
				if ($validator->passes()) 
				{
					...
					$number_of_items =  count($data['family_pricing']['number_of_people']);
					$familyData = [];
				    for($i=0; $i <= $number_of_items; $i++ ){
				        $familyData[] = [
				            'number_of_people' => $data['family_pricing']['number_of_people'][$i],
				            'price' => $data['family_pricing']['price'][$i]
				            ];
				    }
				    $data['family_pricing'] = serialize($familyData);
	...
	public static function display(  )
	{
		...
		$model  = new Insurancepackage();
		$info = $model::makeInfo('insurancepackage');
		$data = array(
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('insurancepackage.public.view',$data);			
			}			
		} 
		else {
			...
			return view('insurancepackage.public.index',$data);	
		}
