...
public static function alphaID($in, $to_num = false, $pad_up = false, $passKey = null)
{
	...
	public static function toMasterDetail( $info )
	{
	 	...
	 	\$this->data['subform'] = \$this->detailview(\$this->modelview ,  \$this->info['config']['subform'] ,\$id );";
		 	...
		 	$tpl = array();
		 	require_once('../resources/views/concave/module/template/native/masterdetailform.php');
	...
	public static function showMaps( $val ){
		...
		if(count($val) ==2) {
			...
			return '<iframe
			  ...
			  src="https://www.google.com/maps/embed/v1/place?key='.config("concave.cnf_maps").'&q='.$lang.','.$long.'" allowfullscreen>
			</iframe>';		
	...
	public static function showFormMaps( $val , $id  ){
		...
		require_once('../resources/views/concave/module/utility/maps.php');
	}
	...
	public static function formatRows( $value ,$attr , $row = null )
	{
		...
		if($format_as =='image')
		{
			...
			if($format_value =='')
			{
				if($concaveconfig['cnf_date'] !='' )
				{
					$value = date("".$concaveconfig['cnf_date']."",strtotime($value));
				} 
			} else {
	...
	{
		$path = base_path().'/resources/views/concave/module/template/';
		$fld = scandir($path);
	...
	public static function BBCode2Html($text) {
		...
		$emotion =  url('concave/js/plugins/markitup/images/emoticons/');
  ...
  static function ResumeUserStatus()
  {
	...
	static public function buttonAction( $module , $access , $id , $setting)
	{
		...
		if($access['is_detail'] ==1) {
			if($setting['view-method'] != 'expand')
			{
				...
				if($setting['view-method'] =='modal')
						$onclick = " onclick=\"ConcaveModal(this.href,'View Detail'); return false; \"" ;
		...
		if($access['is_edit'] ==1) {
			...
			if($setting['form-method'] =='modal')
					$onclick = " onclick=\"ConcaveModal(this.href,'Edit Form'); return false; \"" ;			
	...
	static public function buttonActionCreate( $module  ,$method = 'newpage')
	{
		...
		if($method['form-method'] =='modal')
				$onclick = " onclick=\"ConcaveModal(this.href,'Create Detail'); return false; \"" ;
	...
	static public function htmlExpandGrid()
	{
		...
		return View::make('concave.module.utility.extendgrid');
	}
