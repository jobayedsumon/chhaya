...
@include('concave.config.tab')
 {!! Form::open(array('url'=>'concave/config/save/', 'class'=>'form-horizontal  validated', 'files' => true)) !!}
<div class="p-5">
...
<div class="row">
  	...
  	<div class="col-md-3">
  		<div style="text-align: center; padding-top: 30px;">
      			...
      			<div class="logo-preview preview-upload">
					@if(file_exists(public_path().'/uploads/images/'.$concaveconfig['cnf_logo']) && $concaveconfig['cnf_logo'] !='')
				 		<img src="{{ asset('uploads/images/'.$concaveconfig['cnf_logo'])}}" alt="{{ $concaveconfig['cnf_appname'] }}" width="100" />
				 	@else
						<img src="{{ asset('uploads/logo.png')}}" alt="{{ $concaveconfig['cnf_appname'] }}"  width="100" />
					@endif	
				</div>
	...
	<div class="col-sm-6 animated fadeInRight ">
	  ...
	  <div class="form-group">
		...
		<div class="">
		<input name="cnf_appname" type="text" id="cnf_appname" class="form-control form-control-sm " required  value="{{ $concaveconfig['cnf_appname'] }}" />  
		 </div> 
	  </div>  
	  ...
	  <div class="form-group">
		...
		<div class="">
		<input name="cnf_appdesc" type="text" id="cnf_appdesc" class="form-control form-control-sm" value="{{ $concaveconfig['cnf_appdesc'] }}" /> 
		 </div> 
	  </div>  
	  ...
	  <div class="form-group">
		...
		<div class="">
		<input name="cnf_comname" type="text" id="cnf_comname" class="form-control form-control-sm" value="{{ $concaveconfig['cnf_comname'] }}" />  
		 </div> 
	  </div>      
	  ...
	  <div class="form-group">
		...
		<div class="">
		<input name="cnf_email" type="text" id="cnf_email" class="form-control form-control-sm" value="{{ $concaveconfig['cnf_email'] }}" /> 
		 </div> 
	  </div>   
	  <div class="form-group">
		...
		<div class="">
			<div class="">
				...
				@if($concaveconfig['cnf_multilang'] ==1) checked @endif
	  ...
	  <div class="form-group">
		...
		<div class="">
			<div class="">
				...
				@if($concaveconfig['cnf_front'] =='true') checked @endif
				  /> <label> {{ Lang::get('core.fr_enable') }} </label>
			</div>	
		 </div> 
	  </div> 
	   ...
	   <div class="form-group">
		...
		<div class="">
				...
				@foreach(SiteHelpers::langOption() as $lang)
					...
					@if(config('concave.cnf_lang') ==$lang['folder']) selected @endif
					>{{  $lang['name'] }}</option>
				@endforeach
			</select>
		 </div> 
	   ...
	   <div class="form-group">
		...
		<div class="">
				...
				@foreach(SiteHelpers::themeOption() as $t)
					...
					@if($concaveconfig['cnf_theme'] ==$t['folder']) selected @endif
					>{{  $t['name'] }}</option>
				@endforeach
			</select>
		 </div> 
	   ...
	   <div class="form-group" style="display: none;">
		...
		<div class="">
				...
				@foreach(SiteHelpers::backendOption() as $t)
					<option value="{{  $t['folder'] }}"
					@if($concaveconfig['cnf_backend'] ==$t['folder']) selected @endif
					>{{  $t['name'] }}</option>
				@endforeach
			</select>
		 </div> 
	...
	<div class="col-sm-6 animated fadeInRight ">
	  ...
	  <div class="form-group">
		...
		<div class="">
			...
			foreach($dates as $key=>$val) {?>
				<option value="{{  $key }}"
				@if(config('concave.cnf_date') ==$key) selected @endif
				>{{  $val }}</option>
	  ...
	  <div class="form-group">
		...
		<div class="">
			<textarea class="form-control form-control-sm" name="cnf_metakey">{{ $concaveconfig['cnf_metakey'] }}</textarea>
		 </div> 
	  </div> 
	   ...
	   <div class="form-group">
		...
		<div class="">
			<textarea class="form-control form-control-sm"  name="cnf_metadesc">{{ $concaveconfig['cnf_metadesc'] }}</textarea>
		 </div> 
	  </div>  
	  ...
	  <div class="form-group hide">
		...
		<div class="">
			<div class="checkbox">
				...
				@if ($concaveconfig['cnf_mode'] =='production') checked @endif
				  />  Production
			</div>
