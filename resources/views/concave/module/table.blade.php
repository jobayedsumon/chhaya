...
@include('concave.module.tab',array('active'=>'table','type'=>$type))
<div class="p-3" >
 {!! Form::open(array('url'=>'concave/module/savetable/'.$module_name, 'class'=>'form-horizontal','id'=>'grTable')) !!}
			...
			<?php usort($tables, "SiteHelpers::_sort"); ?>
			  ...
			  <tr >
				...
				<span class=" xlick " title="Lookup Display" 
					onclick="ConcaveModal('{{ URL::to('concave/module/conn/'.$row->module_id.'?field='.$rows['field'].'&alias='.$rows['alias']) }}' ,' Connect Field : {{ $rows['field']}} ' )"
					>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    <?php echo ConcaveHelpers::sjForm('grTable'); ?>
