            ...
            <div class="navbar-header">
                ...
                <div class="top-left-part">
                    ...
                    <span class="hidden-xs">
                         ...
                         @if(file_exists(public_path().'/assets/plugins/images/'.$concaveconfig['cnf_logo']) && $concaveconfig['cnf_logo'] !='')
                        <img src="{{ asset('assets/plugins/images/'.$concaveconfig['cnf_logo'])}}" alt="{{ $concaveconfig['cnf_appname'] }}"  />
                        @else
                        <img src="{{ asset('assets/plugins/images/logo.png')}}" alt="{{ $concaveconfig['cnf_appname'] }}"  />
                        @endif 
                    </span> 
                            ...
                            <li class="col-sm-3">
                                <ul>
                                    <li class="dropdown-header"> Control Panel </li>
                                     <li><a href="{{ url('concave/config')}}"><i class="icon-screen-desktop"></i> {{ Lang::get('core.tab_siteinfo') }}</a> </li> 
                                     <li><a href="{{ url('concave/config/email')}}"><i class="icon-link"></i> {{ Lang::get('core.tab_email') }}</a> </li> 
                                     <li><a href="{{ url('concave/config/security')}}"><i class="icon-lock"></i> {{ Lang::get('core.tab_loginsecurity') }}</a> </li> 
                                     <li><a href="{{ url('concave/config/translation')}}"><i class="icon-map"></i> {{ Lang::get('core.tab_translation') }}</a> </li>  
                                     <li> <a href="{{ url('concave/config/clearlog')}}" class="clearCache"><i class="icon-trash"></i> Clear Log & Caches </a> </li>
                           ...
                           @if(Auth::user()->group_id == 1 or Auth::user()->group_id == 2 )
                            ...
                            <li class="col-sm-3">
                                <ul>
                                    ...
                                    <li><a href="{{ url('concave/module')}}"><i class="icon-fire"></i> Module {{ Lang::get('core.m_codebuilder') }}</a>  </li>
                                    <li><a href="{{ url('concave/menu')}}"><i class="icon-menu"></i> {{ Lang::get('core.m_menu') }}</a> </li>              
                                    <li><a href="{{ url('concave/tables')}}"><i class="fa fa-database"></i> {{ Lang::get('core.m_database') }} </a> </li>
                                    <li> <a href="{{ url('concave/code')}}"><i class="icon-note"></i> {{ Lang::get('core.m_sourceeditor') }}</a>  </li>            
                                    <li><a href="{{ url('core/forms')}}"><i class="icon-list"></i> {{ Lang::get('core.m_formbuilder') }}</a> </li>
                                    <li ><a href="{{ url('concave/rac')}}"><i class="icon-shuffle"></i> RestAPI Generator </a> </li>
                ...
                <ul class="nav navbar-top-links navbar-right pull-right">
                    <!-- /.Task dropdown -->
                    <!-- /.dropdown -->
                     @if($concaveconfig['cnf_multilang'] ==1)
                    <li class="dropdown tasks-menu">
                      <?php 
                      $flag ='en';
                      $langname = 'English'; 
                      foreach(SiteHelpers::langOption() as $lang):
                        if($lang['folder'] == session('lang') or $lang['folder'] == $concaveconfig['cnf_lang']) {
                          $flag = $lang['folder'];
                          $langname = $lang['name']; 
                        }
