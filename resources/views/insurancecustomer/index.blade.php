...
<div class="toolbar-nav" >   
	<div class="row">
		<div class="col-md-8"> 	
			@if($access['is_add'] ==1)
			<a href="{{ url('insurancecustomer/create?return='.$return) }}" class="btn btn-default btn-sm"  
...
<div class="table-container">
			...
			<!-- Table Grid -->
 			...
 			{!! Form::open(array('url'=>'insurancecustomer?'.$return, 'class'=>'form-horizontal m-t' ,'id' =>'ConcaveTable' )) !!}
		    ...
		    <table class="table  table-hover " id="{{ $pageModule }}Table">
		        ...
		        <tbody>        						
		            @foreach ($rowData as $row)
					 <?php 
						 if($row->group_id != 7) continue;
						 if(\Auth::user()->group_id == 5){
							if($row->agent_id != \Auth::id()) continue;
						 }	 
					 ?>
		                <tr>
							<td class="thead"> {{ ++$i }} </td>
							...
							<td>
							 	...
							 	<div class="dropdown">
								  ...
								  <ul class="dropdown-menu">
									...
									<li class="nav-item"><a href="{{ url('insurancecustomer/'.$row->id.'?return='.$return)}}" class="nav-link tips" title="{{ __('core.btn_view') }}"> {{ __('core.btn_view') }} </a></li>
									@endif
									@if($access['is_edit'] ==1)
									<li class="nav-item"><a  href="{{ url('insurancecustomer/'.$row->id.'/edit?return='.$return) }}" class="nav-link  tips" title="{{ __('core.btn_edit') }}"> {{ __('core.btn_edit') }} </a></li>
									@endif
									<div class="dropdown-divider"></div>
									@if($access['is_remove'] ==1)
										<li class="nav-item"><a href="javascript://ajax"  onclick="ConcaveDelete();" class="nav-link  tips" title="{{ __('core.btn_remove') }}">
						 ...
						 @foreach ($tableGrid as $field)
							 ...
							 @if($field['view'] =='1')
								 ...
								 <?php if(\Auth::user()->group_id == 5){
									  if($field['field'] == 'agent_id') continue;
								 } ?>
